...
export function SnsSqsFanoutDiagram() {
  ...
  const getStatusText = () => {
    ...
    if (arch === "fanout") {
      switch (stage) {
        case 1: return "📨 메시지 수신 → SNS Topic 발행";
        case 2: return "📬 SNS → 각 SQS 큐에 Fan-out";
        case 3: return "⚙️ 각 Consumer가 독립적으로 처리";
        case 4: return `✅ ${messageCount.toLocaleString()}건/초 안정 처리!`;
  ...
  const renderDiagram = (isMobile: boolean) => {
    ...
    return (
      ...
      >
        ...
        {arch === "fanout" ? renderFanout(isMobile, width, height) : renderDirect(isMobile, width, height)}
      </svg>
    );
  };
  ...
  const renderDirect = (isMobile: boolean, w: number, h: number) => {
    ...
    return (
      <g>
        {/* Ingestion App */}
        <foreignObject x={isMobile ? cx - 60 : 60} y={isMobile ? appY - 30 : appY - 35} width="120" height="70" className="overflow-visible">
          <div className="w-full h-full flex items-center justify-center">
            <div className={`flex flex-col items-center p-3 rounded-xl border-2 shadow-lg bg-white dark:bg-slate-900 transition-all
              ...
              <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300 mt-1">Ingestion App</span>
              {arch === "direct" && <span className="text-[9px] text-red-500 font-bold">100k msg/s</span>}
            </div>
          </div>
        </foreignObject>
        ...
        {consumers.map((name, i) => {
          ...
          return (
            <g key={name}>
              ...
              />
              <foreignObject x={consumerX - 50} y={consumerY - 25} width="100" height="55" className="overflow-visible">
                <div className="w-full h-full flex items-center justify-center">
                  <div className={`flex flex-col items-center p-2 rounded-lg border-2 shadow-md bg-white dark:bg-slate-900 transition-all text-center
                    ${isOverloaded ? "border-red-300 opacity-60" : "border-slate-200 dark:border-slate-700"}`}>
                    <Users size={16} className={`${isOverloaded ? "text-red-400" : "text-slate-400"}`} />
  ...
  const renderFanout = (isMobile: boolean, w: number, h: number) => {
    ...
    return (
      <g>
        {/* App → SNS line */}
        <line x1={appPos.x + (isMobile ? 0 : 50)} y1={appPos.y + (isMobile ? 30 : 0)} x2={snsPos.x - (isMobile ? 0 : 50)} y2={snsPos.y - (isMobile ? 30 : 0)}
          strokeWidth="3" className={`transition-all duration-500 ${stage >= 1 ? "stroke-amber-400" : "stroke-slate-300 dark:stroke-slate-600"}`} />

        {/* SNS → SQS lines */}
        {sqsBasePos.map((pos, i) => (
          <line key={`sns-sqs-${i}`}
            x1={snsPos.x + (isMobile ? 0 : 40)} y1={snsPos.y + (isMobile ? 30 : 0)}
            x2={pos.x - (isMobile ? 50 : 40)} y2={pos.y}
            strokeWidth="2.5"
        ...
        {consumerPos.map((pos, i) => (
          <line key={`sqs-con-${i}`}
            x1={sqsBasePos[i].x + (isMobile ? 50 : 40)} y1={sqsBasePos[i].y}
            x2={pos.x - (isMobile ? 50 : 40)} y2={pos.y}
            strokeWidth="2"
        ...
        {/* Ingestion App */}
        <foreignObject x={appPos.x - 55} y={appPos.y - 30} width="110" height="60" className="overflow-visible">
          <div className="w-full h-full flex items-center justify-center">
            <div className={`flex flex-col items-center p-2 rounded-xl border-2 shadow-lg bg-white dark:bg-slate-900 transition-all
              ${stage >= 1 ? "border-amber-400 scale-105" : "border-slate-200 dark:border-slate-700"}`}>
              <Zap size={20} className="text-amber-500" />
              <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300">Ingestion App</span>
            </div>
          </div>
        </foreignObject>

        {/* SNS Topic */}
        <foreignObject x={snsPos.x - 55} y={snsPos.y - 32} width="110" height="64" className="overflow-visible">
          <div className="w-full h-full flex items-center justify-center">
            <div className={`flex flex-col items-center p-2.5 rounded-xl border-[3px] shadow-xl bg-white dark:bg-slate-900 transition-all
              ...
              <Bell size={22} className={`transition-all ${stage >= 1 ? "text-orange-500" : "text-slate-400"}`} />
              <span className="text-[10px] font-bold text-orange-600 dark:text-orange-400 mt-0.5">SNS Topic</span>
            </div>
          </div>
        </foreignObject>
        ...
        {consumers.map((c, i) => (
          <g key={c.name}>
            {/* SQS */}
            <foreignObject x={sqsBasePos[i].x - (isMobile ? 48 : 38)} y={sqsBasePos[i].y - 22} width={isMobile ? 96 : 76} height="44" className="overflow-visible">
              <div className="w-full h-full flex items-center justify-center">
                <div className={`flex flex-col items-center p-1.5 rounded-lg border-2 shadow-md bg-white dark:bg-slate-900 transition-all
                  ${stage >= 2 ? `${c.border} scale-105` : "border-slate-200 dark:border-slate-700"}`}>
                  <Inbox size={16} className={`transition-all ${stage >= 2 ? c.color : "text-slate-400"}`} />
                  <span className="text-[8px] font-bold text-slate-600 dark:text-slate-400">SQS {String.fromCharCode(65 + i)}</span>
                </div>
              </div>
            </foreignObject>
            {/* Consumer */}
            <foreignObject x={consumerPos[i].x - (isMobile ? 48 : 45)} y={consumerPos[i].y - 22} width={isMobile ? 96 : 90} height="44" className="overflow-visible">
              <div className="w-full h-full flex items-center justify-center">
                <div className={`flex flex-col items-center p-1.5 rounded-lg border-2 shadow-md bg-white dark:bg-slate-900 transition-all
                  ${stage >= 3 ? `${c.border} scale-105` : "border-slate-200 dark:border-slate-700"}`}>
                  <Users size={14} className={`transition-all ${stage >= 3 ? c.color : "text-slate-400"}`} />
                  <span className="text-[8px] font-bold text-slate-600 dark:text-slate-400">{c.name}</span>
                </div>
              </div>
            </foreignObject>
          </g>
        ...
        {/* Status */}
        {arch && (
          <foreignObject x={isMobile ? 30 : 200} y={isMobile ? 470 : 295} width={isMobile ? 300 : 320} height="40" className="overflow-visible">
            <div className="w-full h-full flex items-center justify-center">
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl shadow-lg border-2 text-xs font-bold
                ${stage >= 4 ? "bg-green-50 border-green-200 text-green-800 dark:bg-green-900/50 dark:border-green-700 dark:text-green-200"
        ...
        <div className="flex-1 p-6 md:p-8 flex flex-col bg-white dark:bg-slate-950">
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl text-xs text-slate-600 dark:text-slate-400 leading-relaxed mb-3">
              <span className="font-bold text-slate-900 dark:text-slate-200 block mb-1">💡 SNS + SQS Fan-out 패턴</span>
              SNS Topic으로 발행하면 구독된 <strong>모든 SQS 큐</strong>에 동시 전달. 각 Consumer는 독립적으로 자기 큐에서 메시지를 소비합니다.
            </div>

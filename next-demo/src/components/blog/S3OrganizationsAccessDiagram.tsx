      ...
      >
        <defs>
          <pattern id="grid-sm" width="16" height="16" patternUnits="userSpaceOnUse">
            <path d="M 16 0 L 0 0 0 16" fill="none" stroke="currentColor" strokeWidth="0.5" className="text-slate-200 dark:text-slate-800" />
          </pattern>
        </defs>
          ...
          <foreignObject x={pos.orgBox.x} y={pos.orgBox.y - 24} width={pos.orgBox.w} height="60">
            <div className="flex justify-center items-center h-full">
              <div className="bg-indigo-100 dark:bg-indigo-900 border-2 border-indigo-200 dark:border-indigo-700 rounded-full px-5 py-1.5 flex items-center gap-2 shadow-sm">
                ...
                {/* 폰트 크기 증가 */}
                <span className="text-sm font-black text-indigo-800 dark:text-indigo-200">AWS Organization (o-abcd123)</span>
              </div>
            </div>
          </foreignObject>
        </g>
        ...
        >
          <div className="w-full h-full flex items-center justify-center p-4">
            {/* 카드 크기 w-[140px]로 확대 */}
            <div className={`w-[140px] flex flex-col items-center p-3 rounded-2xl border-2 transition-all duration-300 bg-white dark:bg-slate-900 shadow-lg
            ...
            `}>
              <div className={`p-3 rounded-full mb-2 ${accessType === 'external' ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500'}`}>
                {/* 아이콘 크기 32px */}
                <ShieldAlert size={32} />
              </div>
              <span className="text-sm font-black text-slate-800 dark:text-slate-200">외부 계정</span>
        ...
        >
          <div className="w-full h-full flex items-center justify-center p-4">
            ...
            `}>
              <div className={`p-3 rounded-full mb-2 ${accessType === 'internal' ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-500'}`}>
                <User size={32} />
              </div>
              <span className="text-sm font-black text-slate-800 dark:text-slate-200">내부 계정</span>
        ...
        >
          <div className="w-full h-full flex items-center justify-center p-4">
            ...
            <div className="w-[160px] flex flex-col items-center p-4 rounded-2xl border-[3px] border-orange-300 bg-white dark:bg-slate-900 shadow-xl relative overflow-hidden group">
              ...
              <span className="text-base font-black text-slate-800 dark:text-slate-100">Target S3</span>
              <div className="mt-2 text-xs bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg text-center w-full">
                <span className="block text-slate-500 font-semibold mb-0.5 text-[10px] uppercase tracking-wider">Condition</span>
                <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400 text-[11px]">PrincipalOrgID</span>
              </div>
            </div>
          </div>
        </foreignObject>
        ...
        {isVerifying && accessType && (
          <g>
            <circle r="8" className={accessType === "internal" ? "fill-indigo-500" : "fill-red-500"}>
              <animateMotion dur="1.5s" repeatCount="1" fill="freeze" key={animationKey}>
                <mpath href={accessType === "internal" ? "#p-internal" : "#p-external"} />
              </animateMotion>
            </circle>
            <circle r="16" className={accessType === "internal" ? "fill-indigo-500" : "fill-red-500"} opacity="0.3">
          ...
          >
            <div className="w-full h-full flex items-center justify-center p-2">
              ...
              `}>
                ...
                <div>
                  <p className="font-black text-base">{result === 'allow' ? '접근 허용 (Allow)' : '접근 거부 (Deny)'}</p>
                  <p className="text-xs opacity-90 font-bold mt-0.5">{result === 'allow' ? '조직 ID 일치 확인됨' : '조직 ID 불일치'}</p>
                </div>
              </div>
            </div>
          </foreignObject>
        ...
        <div className="flex-1 p-6 md:p-8 flex flex-col bg-white dark:bg-slate-950 relative z-20">
          <div className="mb-6">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">
              S3 버킷 정책의 <code className="bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded text-indigo-600 font-mono font-bold">aws:PrincipalOrgID</code> 조건을 검증합니다.
            </p>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
             <div className="bg-slate-50 dark:bg-slate-900 p-3.5 rounded-xl text-xs text-slate-600 dark:text-slate-400 leading-relaxed mb-3">
               <span className="font-bold text-slate-900 dark:text-slate-200 block mb-1">💡 동작 원리</span>

      ...
      services={{
        ...
        others: [
          ...
          },
          {
            ...
            items: [
              "객체 스토리지 (파일 시스템 X)",
              "대용량 파일 저장에 적합, 직접 마운트 불가",
            ],
          },
        ],
        insight: (
          <p className="text-xs md:text-sm text-blue-700 dark:text-blue-300">
            ✨ <strong>핵심 인사이트:</strong> "다중 EC2" + "파일 공유" + "일관된 데이터" = <br />
          ...
          <div className="space-y-4 pt-8">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 bg-red-50 dark:bg-red-900/20 p-4 rounded-xl border border-red-200 dark:border-red-800">
                <div className="w-8 h-8 rounded-full bg-red-100 dark:bg-red-900 flex items-center justify-center font-bold text-red-600 shrink-0">!</div>
                <div>
                  <p className="font-bold text-red-800 dark:text-red-200">문제 상황</p>
                  <p className="text-sm text-red-700 dark:text-red-300">EBS는 각 EC2에 독립적으로 연결 → 문서가 분산 저장됨</p>
                </div>
              </div>
              <div className="flex items-start gap-3 bg-green-50 dark:bg-green-900/20 p-4 rounded-xl border border-green-200 dark:border-green-800">
                <div className="w-8 h-8 rounded-full bg-green-100 dark:bg-green-900 flex items-center justify-center font-bold text-green-600 shrink-0">✓</div>
                <div>
                  <p className="font-bold text-green-800 dark:text-green-200">해결책</p>
                  <p className="text-sm text-green-700 dark:text-green-300">EFS로 마이그레이션 → 모든 EC2가 동일한 파일 시스템 공유</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <Card className="bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800">
              <CardContent className="p-4">
                <ul className="text-sm space-y-2 text-slate-700 dark:text-slate-300">
                  <li>🔹 <strong>다중 EC2 + 파일 공유</strong> → EFS</li>
                  <li>🔹 <strong>Windows 파일 공유</strong> → FSx for Windows</li>
                  <li>🔹 <strong>고성능 HPC/ML</strong> → FSx for Lustre</li>
                  <li>🔹 <strong>단일 EC2 + 고IOPS</strong> → EBS io1/io2</li>
                  <li>🔹 <strong>정적 파일 배포</strong> → S3 + CloudFront</li>
                </ul>
              </CardContent>
            </Card>
          </div>
